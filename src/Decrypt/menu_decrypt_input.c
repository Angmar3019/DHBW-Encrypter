...
void method_list (int state, int checked) {
    ...
    if (state == checked) {     //Case that the state is in the same position as checked
        for (int i = 0; i< state_before;i++) {
            printf("%s%s\n",box_unchecked, method[line]);
            line++;
        }
        printf("%s%s\n",box_checked_sel, method[line]);
        line++;
...
void menu_decrypt_input (int state, int checked) {
    ...
    case 0: //Enter
        if (state <= method_numb) {
            checked = state;
            menu_decrypt_input(state,checked);
            ...
            input_field_decrypt(state, checked);
            menu_decrypt_options(1, global_options);
        }
