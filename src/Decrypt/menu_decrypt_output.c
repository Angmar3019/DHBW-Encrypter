...
void output_list_decrypt (int state, int checked) {
    ...
    if (state == checked) {     //Case that the state is in the same position as checked
        for (int i = 0; i< state_before;i++) {
            printf("%s%s\n",box_unchecked, output[line]);
            line++;
        }
        printf("%s%s\n",box_checked_sel, output[line]);
        line++;
...
void menu_decrypt_output (int state, int checked) {
    ...
    case 0: //Enter
        if (state <= output_numb) {
            checked = state;
            menu_decrypt_output(state,checked);
